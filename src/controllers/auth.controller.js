...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) {
      user = await FranchiseAdmin.findOne({ email: normalizedEmail });
      role = "FranchiseAdmin";
    }

    if (!user) {
      user = await FrontOffice.findOne({ email: normalizedEmail });
      role = "FrontOffice";
    }

    if (!user) {
      user = await LabTechnician.findOne({ email: normalizedEmail });
      role = "LabTechnician";
    }
